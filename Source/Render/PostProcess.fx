...
float4 PPBurnShader( PS_POSTPROCESS_INPUT ppIn ) : SV_Target
{
	...
	else // Draw burning edges
	{
		...
		float2 CrinkleVector = burnTexture.gb - 0.5f; /*FILTER, not 0, read the comment*/
...
float4 PPDistortShader( PS_POSTPROCESS_INPUT ppIn ) : SV_Target
{
	...
	float2 DistortVector = distortTexture.gb - 0.5f;/*FILTER - not 0, similar to crinkle vector used in burn shader above*/
...
float4 PPSpiralShader( PS_POSTPROCESS_INPUT ppIn ) : SV_Target
{
	...
	float2 CentreOffsetUV = ppIn.UV - CentreUV; /*FILTER - not 0, read comment*/
