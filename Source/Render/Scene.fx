...
float4 PPTintShader( VS_BASIC_OUTPUT vOut ) : SV_Target
{
	...
	float3 tex = SceneTexture.Sample(TrilinearWrap, UVScene) * DiffuseColour;
...
float4 PPCutGlassShader( VS_NORMALMAP_OUTPUT vOut ) : SV_Target
{
	...
	float2 sceneUV = vOut.ProjPos;
	sceneUV.x /= ViewportWidth;
	sceneUV.y /= ViewportHeight; // Not 0, fill in the correct values
	//...
...
float4 PPRetroShader(VS_BASIC_OUTPUT vOut) : SV_Target
{
	//***TODO - Create a float2 called UVScene that contains the UV coordinates of this pixel (simple, following the comment above).
	float2 UVScene = vOut.ProjPos;
	UVScene.x /= ViewportWidth;
	...
	UVScene = floor(UVScene * Pixelation) / Pixelation;
	//***TODO - Sample the texture colour at the location UVSCene and multiply it by DiffuseColour (the colour of the material extracted from the .X file, variable already set up)
	float3 tex = SceneTexture.Sample(PointClamp, UVScene);

	// colour pallet
	// round each part of rgba to reduce pallet
	tex = round(tex * ColourPallet) / ColourPallet;

	return float4(tex, 1.0f);
}

float4 PPInvertShader(VS_BASIC_OUTPUT vOut) : SV_Target
{
	float2 UVScene = vOut.ProjPos;
	UVScene.x /= ViewportWidth;
	UVScene.y /= ViewportHeight;

	float3 ppColour = SceneTexture.Sample(PointClamp, UVScene); /*FILTER - not 0*/
...
// Post-processing shader that tints the scene texture to a given colour
float4 PPGreyNoiseShader(VS_BASIC_OUTPUT vOut) : SV_Target
{
	...
	float3 texColour = SceneTexture.Sample(PointClamp, UVScene);
	float grey = (texColour.r + texColour.g + texColour.b) / 3; /*FILTER - not 0, but a method to convert the r,g & b of texColour to
	...
	float2 noiseUV = UVScene * NoiseScale + NoiseOffset;
